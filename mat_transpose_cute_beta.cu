...
// ncu: 
// sudo LD_LIBRARY_PATH=/usr/local/cuda/lib64:$LD_LIBRARY_PATH $(which ncu) -o trans_report -f trans.out true 2048 4096
#ifdef USE_DP
...
__global__ void transpose_cute(const T* A_ptr, T* B_ptr, int M, int N) {
    ...
    // --------------------SMEM to GMEM-------------
    // 转置 sA 的逻辑视图：(32,32):(32,1) -> (32,32):(1,32)
    Tensor sAt = make_tensor(sA.data(), make_layout(
        make_shape (size<1>(sA), size<0>(sA)),
        make_stride(Int<1>{},    Int<32>{})));  // 列主序
    Tensor thr_sA = local_partition(sAt, ShmThreadLayout{}, idx);
    Tensor thr_gB = local_partition(gB, ShmThreadLayout{}, idx);
...
void cute_transpose_kernel_launch(const T* A_ptr, T* B_ptr, int M, int N) {
    ...
    if (use_swizzle) {
        using SmemLayoutAtom = decltype(composition(Swizzle<2, 2, 2>{},
            make_layout(make_shape(Int<32>{}, Int<BlockN>{}), make_stride(Int<BlockN>{}, Int<1>{}))));  // Atom Shape: (8, 32):(32, 1) swizzle(3, 3, 3)
    ...
    using s2g_thr_layout = decltype(make_layout(make_shape(Int<4>{}, Int<32>{}), 
        make_stride(Int<32>{}, Int<1>{})));  // (ThrM, ThrN)


    dim3 gridDim (N/BlockN, M/BlockM);   // Grid shape corresponds to modes m' and n'
    dim3 blockDim(cosize(s2g_thr_layout{}));

    constexpr int shm_size = cosize(SmemLayout{}) * sizeof(T);

    // copy_if()
    transpose_cute<T, G2SCopyA, SmemLayout, s2g_thr_layout><<<gridDim, blockDim, shm_size>>>(A_ptr, B_ptr, M, N);
